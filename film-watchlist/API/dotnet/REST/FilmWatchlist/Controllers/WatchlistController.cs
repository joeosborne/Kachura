...
{
    // todo: Ideally this DTO would just have MovieId. The Movie prop is here because on "Add", the movie is not in the DB - retrievd vial movie API
    ...
    public class WatchlistController : ControllerBase
    {
        ...
        {
            int movieId = 0;
            if (dto.MovieId == 0 && dto.Movie is not null)
            {
                await _context.Movies.AddAsync(dto.Movie);
                await _context.SaveChangesAsync();
                movieId = dto.Movie.Id;
            }
            else
            {
                movieId = (int)dto.MovieId;
            }
            //int movieId = dto.Movie is not null ? dto.Movie.Id dto.MovieId;
